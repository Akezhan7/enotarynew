...
function initQuantityControls() {
    ...
    quantityButtons.forEach(button => {
        button.addEventListener('click', function() {
            ...
            const valueSpan = control.querySelector('.quantity-value');
            const row = control.closest('div[class*="flex"]');
...
function updateTotal() {
    ...
    // Проходим по всем строкам с товарами
    const sections = document.querySelectorAll('section .bg-white.border');
    
    sections.forEach(section => {
        const rows = section.querySelectorAll('div[class*="flex"]');
        
        rows.forEach(row => {
            const quantityControl = row.querySelector('.quantity-control');
            const checkbox = row.querySelector('.checkbox-custom');
            const priceElement = row.querySelector('p[class*="font-bold"]:last-child');
            
            if (quantityControl && priceElement) {
                // Если есть контроль количества
                const quantity = parseInt(quantityControl.querySelector('.quantity-value').textContent);
            ...
            } else if (checkbox && priceElement && checkbox.classList.contains('checked')) {
